...
import { sanitizeFilename, getTimestamp } from '../save-results.post'

describe('sanitizeFilename', () => {
    it('strips protocol and converts URL to filename', () => {
        expect(sanitizeFilename('https://example.com/page')).toBe('example.com_page')
    })
    ...
    it('ignores query parameters (only hostname + pathname are used)', () => {
        ...
        expect(sanitizeFilename('https://example.com/path?q=1&b=2')).toBe('example.com_path')
    })

    it('collapses consecutive underscores into one', () => {
        // Multiple slashes or special chars in a row become a single underscore
        expect(sanitizeFilename('https://example.com///deep///path')).toBe('example.com_deep_path')
    })

    it('preserves dots and hyphens in the hostname', () => {
        expect(sanitizeFilename('https://sub.example-site.com/page')).toBe('sub.example-site.com_page')
    })
    ...
    it('drops query parameters and fragments from the filename', () => {
        ...
        const result = sanitizeFilename('https://example.com/page?key=value#section')
