...
describe('assertWithinOutput', () => {
    ...
    it('blocks paths outside the output directory', () => {
        expect(() => assertWithinOutput('/etc/passwd')).toThrow('Path must be within output directory')
        expect(() => assertWithinOutput('/tmp/evil')).toThrow('Path must be within output directory')
    })

    it('blocks path traversal attacks', () => {
        const traversal = resolve(OUTPUT_ROOT, '..', '..', 'etc', 'passwd')
        expect(() => assertWithinOutput(traversal)).toThrow('Path must be within output directory')
    })

    it('blocks relative path traversal via ../', () => {
        expect(() => assertWithinOutput(OUTPUT_ROOT + '/../secret')).toThrow('Path must be within output directory')
    })
})
